...
const LabelAssociation_1 = require("baileys/lib/Types/LabelAssociation");
const Utils_1 = require("baileys/lib/Utils");
...
exports.default = (config) => {
    ...
    const logger = config.logger || baileys_2.DEFAULT_CONNECTION_CONFIG.logger.child({ stream: 'in-mem-store' });
    ...
    const bind = (ev) => {
            ...
            for (const update of updates) {
                ...
                if (contacts[update.id]) {
                    ...
                    const contactHashes = await Promise.all(Object.keys(contacts).map(async (contactId) => {
                        const { user } = (0, baileys_3.jidDecode)(contactId);
        ...
        ev.on('messages.upsert', ({ messages: newMessages, type }) => {
            switch (type) {
                ...
                case 'notify':
                    for (const msg of newMessages) {
                        const jid = (0, baileys_3.jidNormalizedUser)(msg.key.remoteJid);
                        const list = assertMessageList(jid);
        ...
        ev.on('messages.update', updates => {
            ...
            for (const { update, key } of updates) {
                const list = assertMessageList((0, baileys_3.jidNormalizedUser)(key.remoteJid));
    ...
    const fromJSON = (json) => {
        ...
        for (const jid in json.messages) {
            ...
            for (const msg of json.messages[jid]) {
                list.upsert(baileys_1.proto.WebMessageInfo.fromObject(msg), 'append');
            }
        }
    };
    return {
